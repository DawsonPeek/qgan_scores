...
class PatchQuantumGenerator(nn.Module):
    ...
    def __init__(self, n_generators, n_a_qubits, n_layers=1, q_delta=1):
        ...
        for i in range(n_layers):
            layers.append(nn.Conv2d(current_channels, 32, kernel_size=3, padding=1))
            layers.append(nn.ReLU())
            current_channels = 32

        layers.append(nn.Conv2d(current_channels, 1, kernel_size=3, padding=1))
    ...
    def forward(self, x):
        ...
        quantum_image = output.view(batch_size, 1, image_size, image_size)

        image = self.cnn(quantum_image)
